...
WARNING_TEXT = "Security Alert: Never provide access to sensitive accounts or data, as malicious web content can hijack Claude's behavior."
...
async def main():
    ...
    with st.sidebar:
        ...
        def _update_proxy_url():
            url = st.session_state.proxy_url.strip()
            if url:
        ...
        st.text_input(
            "Proxy URL (optional)",
            key="proxy_url",
            help="If using the OAuth proxy, enter its URL (e.g. http://localhost:9090/proxy/anthropic). Leave blank to call Anthropic directly.",
            on_change=_update_proxy_url,
        )
...
def _render_message(
    ...
    with st.chat_message(sender):
        if is_tool_result:
            ...
            if message.base64_image and not st.session_state.get(
                "hide_images", False
            ):
